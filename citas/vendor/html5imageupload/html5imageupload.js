...
(function (window, $, undefined) {
	...
	$.html5imageupload = function html5imageupload(options, element) {
		...
		this.button.saving		= '<div class="btn btn-warning saving">' + (this.options.saveLabel || 'Guardando...') + ' <i class="fa fa-clock-o"></i></div>';
		this.button.zoomin		= '<div class="btn btn-primary btn-zoom-in" title="' + (this.options.zoominTitle || 'Zoom in') + '"><i class="fa fa-search-plus"></i></div>';
		this.button.zoomout		= '<div class="btn btn-primary btn-zoom-out" title="' + (this.options.zoomoutTitle || 'Zoom out') + '"><i class="fa fa-search-minus"></i></div>';
		this.button.zoomreset	= '<div class="btn btn-primary btn-zoom-reset" title="' + (this.options.zoomresetTitle || 'Fullsize') + '"><i class="fa fa-arrows-alt"></i></div>';
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response.status == "success") {
						...
						if (options.canvas != true) {
							$(element).append($('<img src="' + file + '" class="final" style="width: 100%" />'));
						}
